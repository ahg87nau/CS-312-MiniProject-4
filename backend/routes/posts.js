...
export function makePostsRouter(dao) {
  ...
  router.get("/", async (req, res) => {
    ...
    const posts = await dao.listPosts(category); //  pass string, not object
  ...
  router.get("/:id", async (req, res) => {
    const post = await dao.getPost(req.params.id); //  now defined in DAO
  ...
  router.put("/:id", requireAuth, async (req, res) => {
    const post = await dao.getPost(req.params.id);
    if (!post) return res.status(404).json({ error: "Not found" });
  ...
  router.get("/by/me/all", requireAuth, async (req, res) => {
    const posts = await dao.listUserPosts(req.session.user.user_id); //  matches memoryDAO
    return res.json(posts);
  });
